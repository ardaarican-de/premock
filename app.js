(async function(){
  ...
  function setSolidScene(color,fromHex){
    ...
    if(!fromHex && hexInput) hexInput.value=color.replace(/^#/,'');
    scene.className='';
  ...
  // Lazy-load gallery preview iframes: each card's (possibly heavy Figma embed or large
  // srcdoc) frame only loads once it scrolls into the viewport. A long gallery — or one
  // that's still off-screen in the closed sheet — no longer spins up every iframe at once.
  const galIO = ('IntersectionObserver' in window) ? new IntersectionObserver((entries,obs)=>{
    entries.forEach(en=>{ if(en.isIntersecting){ const f=en.target; if(f._load) f._load(); obs.unobserve(f); } });
  }, {rootMargin:'200px 0px'}) : null;
  function render(){
    if(galIO) galIO.disconnect();   // drop observers from the previous render before rebuilding
    galGrid.innerHTML='';
    ...
    items.forEach((it,i)=>{
      ...
      f._load = (it.type==='url') ? ()=>{ f.src=displaySrcFor(it); } : ()=>{ f.srcdoc=fixedPreviewHTML(it.src); };
  ...
  // Single source of truth for the scene's shareable state (device / status bar / ink /
  // backdrop). Both the query-string link and the Firestore mapping read from here, so the
  // two share paths can't drift apart.
  function sceneState(){
    return {
  ...
  function buildShareLink(){
    ...
    if(st.statusbar) p.set('sb','1');
    if(st.sbink==='light') p.set('bi','l');
    else if(st.sbink==='dark') p.set('bi','d');
    if(st.bg!=='S0') p.set('b', st.bg);
  ...
  // Screen recording + scene capture live in recording.js — a self-contained subsystem that
  // shares no state with this file (it talks to the app only through the DOM).
